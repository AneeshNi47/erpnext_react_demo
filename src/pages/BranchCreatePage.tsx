...
export default function BranchCreatePage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      await erpRequest("api/resource/Branch", {
        method: "POST",
        body: JSON.stringify({ branch, branch_name: branch, company: company || undefined }),
      });
      alert("Branch created");
      setBranch(""); setCompany("");
    } catch (e: any) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-3">
      <div>
        ...
        <input className="border rounded-xl px-3 py-2 w-full" value={branch} onChange={(e)=>setBranch(e.target.value)} required />
      </div>
      <div>
